...
function rollOP(attrVal) {
  ...
  return { rolls, result, worst: attrVal === 0 };
}
...
function FullSheet({ character, onBack }) {
  ...
  const rollFreeInput = () => {
    ...
    setRollPopup({ attr:diceInput.toUpperCase(), rolls, result:rolls.reduce((a,b)=>a+b,0)+mod, worst:false });
    setTimeout(()=>setRollPopup(null),4500);
  };
    ...
    return (
      <div style={{marginBottom:10}}>
        ...
        <div style={{position:"relative",height:34,borderRadius:4,overflow:"hidden",background:"rgba(0,0,0,0.6)",border:"1px solid rgba(255,255,255,0.05)"}}>
          <div style={{position:"absolute",left:0,top:0,bottom:0,width:`${pct*100}%`,background:fill,transition:"width 0.3s, background 0.4s",minWidth:val>0?3:0}}/>
          <div style={{position:"relative",zIndex:1,display:"flex",alignItems:"center",height:"100%"}}>
            ...
            <div style={{flex:1,textAlign:"center",fontFamily:"Cinzel,serif",fontSize:13,fontWeight:700,color:"white",textShadow:"0 1px 4px rgba(0,0,0,0.9)"}}>{val} / {max}</div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:0,position:"relative",fontFamily:"Crimson Pro,serif"}}>
      ...
      {rollPopup && (
        <div style={{position:"fixed",bottom:16,right:16,zIndex:9999,background:"var(--card)",border:"1px solid rgba(201,168,76,0.5)",borderRadius:10,padding:"12px 16px",minWidth:190,boxShadow:"0 6px 32px rgba(0,0,0,0.9)",animation:"fadeIn 0.25s ease",display:"flex",gap:10,alignItems:"center"}}>
          <div style={{width:32,height:32,borderRadius:6,background:"rgba(201,168,76,0.12)",border:"1px solid rgba(201,168,76,0.4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:15,flexShrink:0}}>⬡</div>
          <div style={{flex:1}}>
              ...
              return (
                <div key={p.n}
                  ...
                  onClick={()=>{
                    ...
                    setRollPopup({attr:`${p.n.replace(/[*+]/g,"")} (${p.attr})`,rolls:res.rolls,result:total,worst:res.worst});
                    setTimeout(()=>setRollPopup(null),4500);
                  }}>
                  ...
                  <div key={i} style={{background:"var(--card2)",border:"1px solid var(--border)",borderRadius:6,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
                    <div style={{width:22,height:22,background:"rgba(122,95,212,0.15)",border:"1px solid rgba(122,95,212,0.3)",borderRadius:4,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer"}}
                      onClick={()=>{
                        ...
                        if(m){const n=parseInt(m[1]||"1"),d=parseInt(m[2]),mod=parseInt(m[3]||"0");const rolls=Array.from({length:n},()=>Math.floor(Math.random()*d)+1);setRollPopup({attr:atk.name,rolls,result:rolls.reduce((a,b)=>a+b,0)+mod,worst:false});setTimeout(()=>setRollPopup(null),4500);}
                      }}>
    ...
    <>
      ...
      <div style={{display:"flex", minHeight:"100vh", background:"var(--bg)", position:"relative", zIndex:1}}>
        ...
        <div style={{flex:1, display:"flex", flexDirection:"column", minWidth:0, overflow:"hidden"}}>
          ...
          <div style={{borderTop:"1px solid var(--border)", padding:"8px 20px", display:"flex", gap:12, alignItems:"center"}}>
            <div style={{display:"flex", gap:8, alignItems:"center"}}>
              <NexusLogo size={14}/>
              <span style={{fontFamily:"Cinzel,serif", fontSize:7, letterSpacing:2, color:"var(--muted)", textTransform:"uppercase"}}>Nexus RPG · v0.1 Beta</span>
            </div>
            <div style={{marginLeft:"auto", display:"flex", gap:12}}>
              {["Suporte","Discord","Changelog"].map(l=>(
                <span key={l} style={{fontFamily:"Cinzel,serif", fontSize:7, letterSpacing:1, color:"var(--muted)", cursor:"pointer", textTransform:"uppercase"}}>{l}</span>
              ))}
            </div>
          </div>
        </div>
