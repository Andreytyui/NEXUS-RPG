...
- [x] **T2 — campaignSync.js (novo, AC-1):** `subscribeCampaignMap(db,id,cbs)` (um onSnapshot na
...
- [x] **T3 — MapEditor/index.jsx modo campanha:** props `campaignId/uid/isMaster/db`;
  localStorage OFF em modo campanha (mantido ON quando `campaignId` ausente — AC-5); hidratação
  inicial + snapshot (mestre: só 1ª carga e ignora eco `hasPendingWrites`; jogador: aplica tudo);
  autosave debounced do mestre (cena + imagens novas); modo viewer (AC-2/AC-3): toolbar/painéis
  ocultos, overlay pan/zoom-only, fog opaca, hidden/spectre filtrados.
- [x] **T4 — App.jsx (AC-4):** `CampaignMapTab` (2676–3046) substituído por launcher (status +
