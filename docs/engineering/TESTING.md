...
|------------------|----------------------------------------------------------------------|--------|
| Unidade          | `npm test -- --watchAll=false`                                       | sempre, rápido |
| Integração       | `npm test -- --watchAll=false --testPathPattern=integration`         | adapters / contratos |
| Aceite (UAT)     | `npm test -- --watchAll=false --testPathPattern=acceptance`          | um por `AC-N` da spec |
| Lint / format    | `npm run build` (CRA inclui ESLint; erro de lint = build falha)     | pré-commit / CI |
| Análise estática | `npm run build` (ESLint CRA — findings de erro são bloqueantes)     | CI |
| Cobertura        | `npm test -- --watchAll=false --ci --coverage` → `coverage/`        | CI — artefato no PR |
